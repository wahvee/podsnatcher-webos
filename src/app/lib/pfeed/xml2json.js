...
(function($) {
	...
	var INodeSet = function() {				
		this.getNodesByAttribute = function(attr, obj){
			var out = [];
		...
		this.getNodesByValue = function(obj){
			var out = [];
		...
		this.contains = function(attr, obj){
			var out = false;
			if(!!this.length && util.isStr(attr) && util.isDef(obj)) {
				var n = this.length, node, aval;
				while(n--) {
		...
		this.sortByAttribute = function(attr, dir){
		...
		this.sortByChildNode = function(nodeName, dir){
